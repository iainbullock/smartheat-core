...
class ThermostatService extends Service {
    ...
    async time() {
        ...
        try {
            ...
            let formatted_device_time = dateFormat(device.time, "h:m");

            let timeDelta = (Date.parse(Date()) - device.time) / 60000;

            const messages = [];
            messages.push(`The device time is ${formatted_device_time}, which is ${timeDelta} minutes fast`);
