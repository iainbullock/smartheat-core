...
class ThermostatService extends Service {
    ...
    async time() {
        ...
        try {
            ...
            let timeDelta = (Date.parse(Date()) - device.time) / 1000;

            const messages = [];
            messages.push(`The device time is ${formatted_device_time}, which is ${Math.abs(timeDelta.toFixed(1))} seconds ${(timeDelta < 0) ? 'fast' : 'slow'}`);
